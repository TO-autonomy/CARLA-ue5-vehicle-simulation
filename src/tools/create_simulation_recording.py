...
class World(object):
    ...
    def spawn_vehicles(self, n, ai_controlled):
        ...
        for _ in range(n):
            ...
            if vehicle and ai_controlled:
                vehicle.set_autopilot(True)

    def spawn_pedestrians(self, n, ai_controlled):
        ...
        for _ in range(n):
            ...
            if pedestrian and ai_controlled:
                controller = self.world.try_spawn_actor(walker_controller_bp, carla.Transform(), pedestrian)
                if controller:
                    controller.start()
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot, recording_path="manual_recording.rec"):
...
def game_loop(args):
    ...
    text_lines = [
        ...
        "- Press CTRL+R again to stop recording.",
        "- Press ESC or Q to quit the simulation.",
        "",
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    waiting = False
                    display.fill(WHITE)
                    text_surface = font.render("Loading simulation...", True, BLACK)
                    text_rect = text_surface.get_rect(center=(WIDTH // 2, HEIGHT // 2))
                    display.blit(text_surface, text_rect)
    
    display.fill(WHITE)
    text_lines = ["Loading simulation..."]
    y_offset = HEIGHT // 2
    for line in text_lines:
        text_surface = font.render(line, True, BLACK)
        text_rect = text_surface.get_rect(center=(WIDTH // 2, y_offset))
    ...
    try:
        ...
        pygame.display.set_caption("CARLA Simulation - Press CTRL+R to Record")
        # display = pygame.display.set_mode(
        #     (args.width, args.height),
        #     pygame.HWSURFACE | pygame.DOUBLEBUF)
        # display.fill((0,0,0))
        ...
        # Spawn AI vehicles and pedestrians
        world.spawn_vehicles(args.ai_vehicles, ai_controlled=True)
        world.spawn_vehicles(args.dormant_vehicles, ai_controlled=False)
        world.spawn_pedestrians(args.ai_pedestrians, ai_controlled=True)
        world.spawn_pedestrians(args.dormant_pedestrians, ai_controlled=False)
...
def main():
    ...
    argparser.add_argument(
        ...
        type=int,
        default=0,
        help='Number of AI-controlled vehicles to spawn')
    argparser.add_argument(
        '--dormant_vehicles',
        type=int,
        default=0,
        help='Number of dormant vehicles to spawn')
    argparser.add_argument(
        '--ai_pedestrians',
        type=int,
        default=0,
        help='Number of AI-controlled pedestrians to spawn')
    argparser.add_argument(
        '--dormant_pedestrians',
        type=int,
        default=0,
